...
import fun.jiucai.home.chatai.silicon.MyQwenOpenAi;
...
public class DeepSeekService {
    ...
    private List<MyQwenOpenAi> deepSeeks = new ArrayList<>();

    private AtomicInteger atomic = new AtomicInteger(0);

    @PostConstruct
    public void init() {
        for (var config : myConfiguration.getDeepseek()) {
            var service = new MyQwenOpenAi("https://api.deepseek.com", config.getToken(), Duration.ofSeconds(30));
            deepSeeks.add(service);
        }
    ...
    public void process(Session session, long requestSid, long requestId, List<ChatgptMessage> messages) {
        var service = deepSeeks.get(Math.abs(atomic.incrementAndGet() % deepSeeks.size()));
        ...
        service.streamChatCompletion(chatCompletionRequest).blockingForEach(new Consumer<ChatCompletionChunk>() {
            ...
            public void accept(ChatCompletionChunk chatCompletionChunk) throws Exception {
                ...
                var chatRequestId = Integer.parseInt(StringUtils.format("{}{}", ChatAIEnum.deepseek.getType(), requestId));
                var finishReason = "stop".equalsIgnoreCase(choice.getFinishReason()) ? ChatgptMessageNotice.STOP : ChatgptMessageNotice.GENERATING;
                var notice = new ChatgptMessageNotice(chatRequestId, ChatAIEnum.deepseek.getType(), content, finishReason);
