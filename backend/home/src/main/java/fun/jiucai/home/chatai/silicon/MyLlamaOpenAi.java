...
package fun.jiucai.home.chatai.silicon;
...
public class MyLlamaOpenAi {
    ...
    public MyLlamaOpenAi(String url, String token, Duration timeout) {
        this.url = url;
        this.token = token;
    ...
    public static Flowable<SSE> stream(retrofit2.Call<ResponseBody> apiCall, boolean emitDone) {
        return Flowable.create(emitter -> apiCall.enqueue(new MyResponseBodyCallback(emitter, emitDone)), BackpressureStrategy.BUFFER);
    }
    ...
    private static final class MyResponseBodyCallback<ResponseBody> extends ResponseBodyCallback {

        private FlowableEmitter<SSE> myEmitter;
        private boolean myEmitDone;

        public MyResponseBodyCallback(FlowableEmitter<SSE> emitter, boolean emitDone) {
            super(emitter, emitDone);
            this.myEmitter = emitter;
            this.myEmitDone = emitDone;
        }
        ...
        public void onResponse(retrofit2.Call<okhttp3.ResponseBody> call, Response<okhttp3.ResponseBody> response) {
            ...
            try {
                ...
                while (!myEmitter.isCancelled() && (line = reader.readLine()) != null) {
                    if (line.startsWith(": ping")) {
                        continue;
                    }
                    if (StringUtils.isEmpty(line) && sse == null) {
                        continue;
                    }
